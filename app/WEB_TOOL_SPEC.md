...
- Single-user/máy. `ailab_projects/` là workspace local do tool sinh ra; phối hợp nhóm bằng **repo dataset riêng**, không commit runtime workspace vào repo công cụ.

---

## 3. Project workspace (filesystem)

`ailab_projects/<doc_id>/` là **dataset project workspace local**. Repo công cụ `ailab-202-handoff` phải ignore toàn bộ `ailab_projects/` để không trộn runtime data vào code. Các quy ước git bên dưới áp dụng cho **repo/thư mục dataset riêng** của nhóm, nơi mỗi `doc_id` do một người phụ trách tại một thời điểm.
...
ailab_projects/
  <doc_id>/
    ...
    canonical/                # 5 FILE DATASET (live, structurally valid) — track trong repo dataset
      document.json
      glossary.jsonl
      entities.jsonl
      chapter_summaries.jsonl
      manual_reference_subset.jsonl
    working/                  # trạng thái làm việc — review_state + draft/export; autosave KHÔNG
...
**Ranh giới git cho repo dataset riêng:** vào git = `project.json`, `raw/` (+ provenance nếu license/file size cho phép), `canonical/`, `working/review_state.json`, `working/drafts.json`, `working/translation_review_log.csv`, project `CHANGELOG.md`, `versions.json`. **gitignore** = `working/autosave/`, `working/history.json`, `logs/`, `exports/`. Nếu raw file quá lớn hoặc license không cho commit, chỉ commit `raw/provenance.json` + `raw_sha256` + hướng dẫn lấy lại nguồn; không commit file raw.
...
| `manual_reference_subset.jsonl` | Draft ở `working/drafts.json` qua UI/API | ✅ **CHỈ reference**: draft → `reviewed` mới promote sang JSONL |
...
`working/translation_review_log.csv` là **bản export một chiều, read-only** sinh tự động từ `working/drafts.json` để xem/QC bằng spreadsheet. App không đọc ngược CSV làm input; mọi sửa reference draft đi qua UI/API và ghi vào `drafts.json`.
...
| `PATCH /projects/{doc_id}` | Sửa thông tin project tối thiểu: `note`; `doc_id` không đổi, tên sách sửa ở metadata |
| `DELETE /projects/{doc_id}` | Xóa project local; bắt buộc `confirm_doc_id` khớp và không cho xóa golden sample |
...
- **Project management tối thiểu:** ghi chú project (`note`) để giao việc/ghi vấn đề nguồn; `doc_id` là định danh/folder nên không rename trực tiếp, tên sách thật nằm ở metadata `title`.
- **Xóa project local:** chỉ dùng cho project nháp/test, có confirm; golden sample bị khóa không cho xóa.
...
- Chạy cleanup deterministic: TXT cắt Gutenberg START/END marker; EPUB bỏ front matter theo OPF guide/nav khi có metadata cấu trúc; `source_format` lấy theo đuôi file thật.
- Chạy `validate.py` sơ bộ sau extraction; ghi `working/extraction_report.json` + job record.
- *Guard:* nếu project đã có glossary/entity/reference/summary hoặc block đã reviewed thì không re-extract mặc định; chỉ force bằng backend flag mạnh vì re-extract có thể làm hỏng block_id/span.
...
- **Lifecycle:** draft sửa qua UI/API và ghi vào `working/drafts.json`; `working/translation_review_log.csv` chỉ là export read-only sinh tự động để xem/QC; **chỉ `reviewed/locked` mới promote sang `manual_reference_subset.jsonl`**.
...
- `working/extraction_report.json`, `validation.log`, `jobs.jsonl` (job record tối giản như §dưới).
...
{"job_id":"extract_2026_06_01_153000","type":"extract","status":"done",
 ...
 "output":"canonical/document.json","report_file":"working/extraction_report.json"}
```
