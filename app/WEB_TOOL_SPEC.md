...
ailab_projects/
  <doc_id>/
    ...
    working/                  # trạng thái làm việc — review_state + CSV vào git; autosave KHÔNG
      ...
      translation_review_log.csv   # reference DRAFT (chưa vào JSONL)
...
**Ranh giới git:** vào git = `project.json`, `raw/` (+ provenance nếu license/file size cho phép), `canonical/`, `working/review_state.json`, `working/translation_review_log.csv`, project `CHANGELOG.md`, `versions.json`. **gitignore** = `working/autosave/`, `logs/`, `exports/`. Nếu raw file quá lớn hoặc license không cho commit, chỉ commit `raw/provenance.json` + `raw_sha256` + hướng dẫn lấy lại nguồn; không commit file raw.
...
| `manual_reference_subset.jsonl` | Draft ở `working/translation_review_log.csv` | ✅ **CHỈ reference**: draft → `reviewed` mới promote sang JSONL |
...
- **Lifecycle:** draft ghi vào `working/translation_review_log.csv`; **chỉ `reviewed/locked` mới `POST /promote` sang `manual_reference_subset.jsonl`**.
